...
控制干扰变量之后，理论型课程对于学习者自主动机有显著正向影响\nolinebreak[4]（$\beta=0.66$，$p<0.001$），说明假设H1a成立。实践型课程也有很强的正向作用\nolinebreak[4]（$\beta=0.61$，$p<0.001$），再次证明研究假设H1b成立。就融合型课程而言，它对学习者自主动机同样具有显著正向影响\nolinebreak[4]（$\beta=0.70$，$p<0.001$），因此研究假设H1c得到支持。

从三类课程的比较来看，融合型课程的标准化回归系数最高\nolinebreak[4]（$\beta=0.70$），其解释力也最强\nolinebreak[4]（$R^2=0.57$），说明融合型课程在激发学生自主动机方面效果最为突出。
...
本研究采用分层回归的方法对外部动机的调节作用进行研究。在模型搭建时，把核心自变量和调节变量进行标准化处理，建立相关的交互作用项\nolinebreak[4]（自主动机$\times$外部动机）。因此，本研究对不同类型课程数据的调节效应做了详细的验证，具体的结果如表\ref{tab:moderation_a}、表\ref{tab:moderation_b}及表\ref{tab:moderation_c}所示。
...
\begin{table}[H]
    \centering
    \caption{外部动机的调节效应检验\nolinebreak[4]（理论型课程，N=68）}\zihao{5}
    \begin{tabular}{lcccccc}
        \toprule
...
\begin{table}[H]
    \centering
    \caption{外部动机的调节效应检验\nolinebreak[4]（实践型课程，N=53）}\zihao{5}
    \begin{tabular}{lcccccc}
        \toprule
        \multirow{2}{*}{\songtibf{变量}} & \multicolumn{2}{c}{\songtibf{自主动机}} & \multicolumn{2}{c}{\songtibf{创业意愿}} & \multicolumn{2}{c}{\songtibf{创业意愿}} \\
        \cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
...
\begin{table}[H]
    \centering
    \caption{外部动机的调节效应检验\nolinebreak[4]（融合型课程，N=33）}\zihao{5}
    \begin{tabular}{lcccccc}
        \toprule
        \multirow{2}{*}{\songtibf{变量}} & \multicolumn{2}{c}{\songtibf{自主动机}} & \multicolumn{2}{c}{\songtibf{创业意愿}} & \multicolumn{2}{c}{\songtibf{创业意愿}} \\
        \cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
