...
各变量的均值、标准差及相关系数如表\ref{tab:correlation}所示。从均值来看，三类课程的感知得分均处于较高水平\nolinebreak[3]（理论型：$M=4.18$，$SD=0.61$；实践型：$M=4.15$，$SD=0.82$；融合型：$M=4.38$，$SD=0.56$），其中融合型课程得分最高；自主动机均值为4.24\nolinebreak[3]（$SD=0.65$），外部动机均值为3.06\nolinebreak[3]（$SD=0.93$），创业意愿均值为3.79\nolinebreak[3]（$SD=0.98$）。

创业意愿同自主动机有明显的正相关联系\nolinebreak[3]（$r=0.776$，$p<0.01$），而且二者同融合型课程以及理论型、实践型课程也存在着显著的正相关表现\nolinebreak[3]（$r=0.589$，0.430，0.406，$p<0.01$）。经过进一步的研究发现，自主动机对于不同的课程类型有较高的依赖性，相关系数都在 $r=0.641$--$0.678$ 之间，有统计学意义。外部动机与其它变量的相关性比较低，大部分没有达到统计学显著水平。上面的结论给后面的研究假设提供了事实依据以及数据来源。
...
控制干扰变量之后，理论型课程对于学习者自主动机有显著正向影响\nolinebreak[3]（$\beta=0.66$，$p<0.001$），说明假设H1a成立。实践型课程也有很强的正向作用\nolinebreak[3]（$\beta=0.61$，$p<0.001$），再次证明研究假设H1b成立。就融合型课程而言，它对学习者自主动机同样具有显著正向影响\nolinebreak[3]（$\beta=0.70$，$p<0.001$），因此研究假设H1c得到支持。

从三类课程的比较来看，融合型课程的标准化回归系数最高\nolinebreak[3]（$\beta=0.70$），其解释力也最强\nolinebreak[3]（$R^2=0.57$），说明融合型课程在激发学生自主动机方面所表现出的整体效果最为突出。

\begin{table}[H]
    \centering
    \caption{主效应检验结果}\zihao{5}
    \begin{tabular}{lcccccc}
        \toprule
        \multirow{2}{*}{\songtibf{变量}} & \multicolumn{2}{c}{\songtibf{理论型课程}} & \multicolumn{2}{c}{\songtibf{实践型课程}} & \multicolumn{2}{c}{\songtibf{融合型课程}} \\
        ...
        \midrule
        $R^2$ & \multicolumn{2}{c}{0.44} & \multicolumn{2}{c}{0.45} & \multicolumn{2}{c}{0.57} \\
        F & \multicolumn{2}{c}{7.92***} & \multicolumn{2}{c}{6.62***} & \multicolumn{2}{c}{5.74***} \\
        \bottomrule
    \end{tabular}
    \par\vspace{2pt}
    \begin{minipage}{0.92\textwidth}\zihao{-5}\raggedright
        注：*$p<0.05$，**$p<0.01$，***$p<0.001$；$\beta$代表标准化的回归系数；SE代表样本标准误。
    \end{minipage}
...
由表\ref{tab:mediation_a}可知，在理论型课程样本中，将性别、年级、专业、创业经历、志愿服务作为控制变量纳入回归模型后，理论型课程对创业意愿的总效应显著\nolinebreak[3]（$\beta=0.43$，$p<0.001$）。当加入中介变量自主动机后，理论型课程的直接影响变得不显著\nolinebreak[3]（$\beta=0.12$，$p>0.05$），而自主动机对创业意愿的预测作用显著\nolinebreak[3]（$\beta=0.47$，$p<0.001$）。中介效应检验结果表明，理论型课程对创业意愿的间接效应值为0.748，95\%置信区间为[0.492, 1.031]，不包含0，说明间接效应显著。上述结果表明自主动机在理论型课程与创业意愿之间起完全中介作用，由此得出H2a假设成立的结论。

由表\ref{tab:mediation_b}可知，在实践型课程样本中，实践型课程对创业意愿的总效应显著\nolinebreak[3]（$\beta=0.41$，$p<0.001$）。当加入中介变量自主动机后，实践型课程的直接影响变得不显著\nolinebreak[3]（$\beta=0.10$，$p>0.05$），而自主动机对创业意愿的预测作用显著\nolinebreak[3]（$\beta=0.45$，$p<0.001$）。实践型课程的间接效应值为0.726，95\%置信区间为[0.270, 1.682]，不包含0，说明间接效应显著。上述结果表明自主动机在实践型课程与创业意愿之间起完全中介作用，由此得出H2b假设成立的结论。

由表\ref{tab:mediation_c}可知，在融合型课程样本中，融合型课程对创业意愿的总效应显著\nolinebreak[3]（$\beta=0.45$，$p<0.001$）。当加入中介变量自主动机后，融合型课程的直接影响变得不显著\nolinebreak[3]（$\beta=0.13$，$p>0.05$），而自主动机对创业意愿的预测作用显著\nolinebreak[3]（$\beta=0.46$，$p<0.001$）。融合型课程的间接效应值为0.857，95\%置信区间为[0.404, 1.615]，不包含0，说明间接效应显著。上述结果表明自主动机在融合型课程与创业意愿之间起完全中介作用，由此得出H2c假设成立的结论。
...
本研究采用分层回归的方法对外部动机的调节作用进行研究。在模型搭建时，把核心自变量和调节变量进行标准化处理，建立相关的交互作用项\nolinebreak[3]（自主动机$\times$外部动机）。因此，本研究对不同类型课程数据的调节效应做了详细的验证，具体的结果如表\ref{tab:moderation_a}、表\ref{tab:moderation_b}及表\ref{tab:moderation_c}所示。

由表\ref{tab:moderation_a}可知，在理论型课程样本中，加入控制变量、理论型课程、自主动机和外部动机后，自主动机与外部动机的交互项对创业意愿的回归系数为-0.148，对应的p值为0.332，未通过显著性检验。因此可以得出假设H3a没有得到足够的证据支持。

由表\ref{tab:moderation_b}可知，在实践型课程样本中，加入控制变量、实践型课程、自主动机和外部动机后，自主动机与外部动机的交互项对创业意愿的回归系数为-0.406，对应的p值为0.089，未通过显著性检验。因此可以得出假设H3b没有得到足够的证据支持。

由表\ref{tab:moderation_c}可知，在融合型课程样本中，加入控制变量、融合型课程、自主动机和外部动机后，自主动机与外部动机的交互项对创业意愿的回归系数为-0.535，对应的p值为0.048，小于0.05，通过了显著性检验。这表明外部动机对融合型课程中自主动机与创业意愿的关系具有显著的负向调节作用，即外部动机越强的学生，自主动机对创业意愿的正向影响越弱。因此，假设H3c成立。

综合表\ref{tab:moderation_a}、表\ref{tab:moderation_b}及表\ref{tab:moderation_c}的分析结果可知，外部动机对自主动机与创业意愿关系的负向调节作用仅在融合型课程中显著\nolinebreak[3]（$\beta=-0.535$，$p=0.048$），在理论型课程\nolinebreak[3]（$\beta=-0.148$，$p=0.332$）和实践型课程\nolinebreak[3]（$\beta=-0.406$，$p=0.089$）中均不显著。因此，假设H3c成立，H3a和H3b不成立。

\begin{table}[H]
    \centering
    \caption{外部动机的调节效应检验\nolinebreak[3]（理论型课程，N=68）}\zihao{5}
    \begin{tabular}{lcccccc}
        \toprule
        \multirow{2}{*}{\songtibf{变量}} & \multicolumn{2}{c}{\songtibf{自主动机}} & \multicolumn{2}{c}{\songtibf{创业意愿}} & \multicolumn{2}{c}{\songtibf{创业意愿}} \\
        \cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
        & $\beta$ & SE & $\beta$ & SE & $\beta$ & SE \\
        \midrule
        ...
        \midrule
        $R^2$ & \multicolumn{2}{c}{0.44} & \multicolumn{2}{c}{0.32} & \multicolumn{2}{c}{0.38} \\
        F & \multicolumn{2}{c}{8.56***} & \multicolumn{2}{c}{5.89***} & \multicolumn{2}{c}{6.87***} \\
        \bottomrule
    \end{tabular}
    \par\vspace{2pt}
    \begin{minipage}{0.92\textwidth}\zihao{-5}\raggedright
        注：*$p<0.05$，**$p<0.01$，***$p<0.001$；$\beta$代表标准化的回归系数；SE代表样本标准误。
    \end{minipage}
    \label{tab:moderation_a}
\end{table}

\begin{table}[H]
    \centering
    \caption{外部动机的调节效应检验\nolinebreak[3]（实践型课程，N=53）}\zihao{5}
    \begin{tabular}{lcccccc}
        \toprule
        \multirow{2}{*}{\songtibf{变量}} & \multicolumn{2}{c}{\songtibf{自主动机}} & \multicolumn{2}{c}{\songtibf{创业意愿}} & \multicolumn{2}{c}{\songtibf{创业意愿}} \\
        \cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
        ...
        \midrule
        $R^2$ & \multicolumn{2}{c}{0.42} & \multicolumn{2}{c}{0.31} & \multicolumn{2}{c}{0.36} \\
        F & \multicolumn{2}{c}{7.89***} & \multicolumn{2}{c}{5.12***} & \multicolumn{2}{c}{6.21***} \\
        \bottomrule
    \end{tabular}
    \par\vspace{2pt}
    \begin{minipage}{0.92\textwidth}\zihao{-5}\raggedright
        注：*$p<0.05$，**$p<0.01$，***$p<0.001$；$\beta$代表标准化的回归系数；SE代表样本标准误。
    \end{minipage}
    \label{tab:moderation_b}
\end{table}

\begin{table}[H]
    \centering
    \caption{外部动机的调节效应检验\nolinebreak[3]（融合型课程，N=33）}\zihao{5}
    \begin{tabular}{lcccccc}
        \toprule
        \multirow{2}{*}{\songtibf{变量}} & \multicolumn{2}{c}{\songtibf{自主动机}} & \multicolumn{2}{c}{\songtibf{创业意愿}} & \multicolumn{2}{c}{\songtibf{创业意愿}} \\
        \cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
        ...
        \midrule
        $R^2$ & \multicolumn{2}{c}{0.46} & \multicolumn{2}{c}{0.33} & \multicolumn{2}{c}{0.39} \\
        F & \multicolumn{2}{c}{9.12***} & \multicolumn{2}{c}{6.01***} & \multicolumn{2}{c}{7.23***} \\
        \bottomrule
    \end{tabular}
    \par\vspace{2pt}
    \begin{minipage}{0.92\textwidth}\zihao{-5}\raggedright
